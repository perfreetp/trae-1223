...
function renderSchedule() {
  ...
  const todoRowLabel = document.createElement('div');
  todoRowLabel.className = 'schedule-account-row schedule-todo-row-label';
  todoRowLabel.innerHTML = `<div style="display:flex;align-items:center;gap:6px;font-weight:600;"><span style="color:#722ED1;">📋</span>本周待办</div>`;
  todoRowLabel.title = '待办不绑定账号，按截止日期显示';
  accountsCol.appendChild(todoRowLabel);
  ...
  const weekTodos = allInteractions.filter(i => i.type === 'todo');
  ...
  for (let i = 0; i < 7; i++) {
    ...
    cell.className = 'schedule-cell schedule-todo-cell';
    if (isToday) cell.classList.add('today');
    cell.style.background = 'rgba(114, 46, 209, 0.02)';
    cell.style.borderTop = '2px dashed rgba(114, 46, 209, 0.3)';
    cell.style.borderBottom = '2px dashed rgba(114, 46, 209, 0.3)';
    const dateTodos = weekTodos.filter(t => t.dueDate === dateStr);
    ...
    if (dateTodos.length === 0) {
      ...
      empty.style.cssText = 'color:var(--text-tertiary);font-size:10px;padding:4px;text-align:center;opacity:0.6;';
      empty.textContent = '—';
